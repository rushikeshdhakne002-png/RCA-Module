...
if page == "Dashboard":
    ...
    if df.empty:
        ...
        # Derived fields
        df["_reported"] = pd.to_datetime(df["Reported Date"])
        today = pd.Timestamp.today().normalize()
        df["_age_days"] = (today - df["_reported"].dt.normalize()).dt.days.clip(lower=0)
        target = pd.to_datetime(df["Target Completion Date"], errors="coerce")
        df["_sla_breach"] = df["Status (Closed / Pending)"].eq("Pending") & target.notna() & (target.dt.normalize() < today)
