...
page = st.sidebar.radio(
    "Go to",
    ["Dashboard", "Issue Log (Quick Add + Editor)", "Manage Issues", "Import / Export"],
    index=0,
)
...
def fetch_filtered():
    ...
    if st.session_state.get("flt_from"):
        stmt = stmt.where(
            Issue.reported_date
            >= datetime.combine(st.session_state["flt_from"], datetime.min.time())
        )
    if st.session_state.get("flt_to"):
        stmt = stmt.where(
            Issue.reported_date
            <= datetime.combine(st.session_state["flt_to"], datetime.max.time())
        )
    stmt = stmt.order_by(
        Issue.reported_date.desc().nulls_last(), Issue.created_at.desc()
    )
...
def parse_dt(x, default_today=False):
    ...
    ) or (hasattr(x, "strip") and x.strip() == ""):
        return (
...
def get_live_df():
    return to_df(fetch_filtered())


def show_filter_summary():
    active = []
    if st.session_state.get("flt_module"):
        active.append(f'Module: "{st.session_state["flt_module"]}"')
    if st.session_state.get("flt_priority"):
        active.append("Priority: " + ", ".join(st.session_state["flt_priority"]))
    if st.session_state.get("flt_status"):
        active.append("Status: " + ", ".join(st.session_state["flt_status"]))
    if st.session_state.get("flt_from"):
        active.append(f'From: {st.session_state["flt_from"]}')
    if st.session_state.get("flt_to"):
        active.append(f'To: {st.session_state["flt_to"]}')
    if active:
...
if page == "Dashboard":
    ...
    if df.empty:
        ...
        # Derived fields – make dates timezone-aware then drop tz
        df["_reported"] = (
            pd.to_datetime(df["Reported Date"], utc=True).dt.tz_convert(None)
        )
        ...
        df["_age_days"] = (
            today - df["_reported"].dt.normalize()
        ).dt.days.clip(lower=0)

        # Target completion date (also normalize tz)
        target = (
            pd.to_datetime(
                df["Target Completion Date"], errors="coerce", utc=True
            ).dt.tz_convert(None)
        )
        ...
        with c1:
            st.markdown(
                f'<div class="kpi {"kpi-v1" if vibrant else ""}"><div class="label">Total</div><div class="value">{total}</div></div>',
                unsafe_allow_html=True,
            )
        with c2:
            st.markdown(
                f'<div class="kpi {"kpi-v2" if vibrant else ""}"><div class="label">Pending</div><div class="value">{pending}</div></div>',
                unsafe_allow_html=True,
            )
        with c3:
            st.markdown(
                f'<div class="kpi {"kpi-v3" if vibrant else ""}"><div class="label">Closed</div><div class="value">{closed}</div></div>',
                unsafe_allow_html=True,
            )
        with c4:
            st.markdown(
                f'<div class="kpi {"kpi-v4" if vibrant else ""}"><div class="label">Critical</div><div class="value">{crit}</div></div>',
                unsafe_allow_html=True,
            )
        with c5:
            st.markdown(
                f'<div class="kpi {"kpi-v5" if vibrant else ""}"><div class="label">SLA Breaches</div><div class="value">{breaches}</div></div>',
                unsafe_allow_html=True,
            )
        with c6:
            st.markdown(
                f'<div class="kpi {"kpi-v6" if vibrant else ""}"><div class="label">Avg Age (days)</div><div class="value">{avg_age}</div></div>',
                unsafe_allow_html=True,
            )
        ...
        with r1c1:
            t = df.copy()
            t["Reported"] = (
                pd.to_datetime(t["Reported Date"], utc=True)
                .dt.tz_convert(None)
                .dt.date
            )
        ...
        with r2c1:
            ...
            fig.update_layout(
                title="Top Modules by Issue Count", margin=dict(l=0, r=0, t=40, b=0)
            )
            st.plotly_chart(fig, use_container_width=True)
        with r2c2:
            sp = df.pivot_table(
                index="Priority",
                columns="Status (Closed / Pending)",
                values="ID",
                aggfunc="count",
                fill_value=0,
            )
            ...
            sp = (
                sp.rename_axis(None, axis=1)
                .reset_index()
                .melt(id_vars="Priority", var_name="Status", value_name="Count")
            )
            fig = px.bar(
                sp,
                x="Priority",
            ...
            fig.update_layout(
                title="Status × Priority", margin=dict(l=0, r=0, t=40, b=0)
            )
            st.plotly_chart(fig, use_container_width=True)
        ...
        with r3c1:
            ...
            fig.update_layout(
                title="Issue Aging (days)", margin=dict(l=0, r=0, t=40, b=0), showlegend=False
            )
            st.plotly_chart(fig, use_container_width=True)
        with r3c2:
            ...
            fig = px.treemap(
                tm,
                path=["Module Name", "Priority"],
                values="Count",
                color="Priority",
                color_discrete_sequence=pie_colors,
            )
            fig.update_layout(
                title="Treemap: Module vs Priority", margin=dict(l=0, r=0, t=40, b=0)
            )
            st.plotly_chart(fig, use_container_width=True)
...
# ---------- Issue Log (Dark) ----------
    ...
    with st.form("quick_add_form"):
        ...
        with q1:
            ticket_id = st.text_input("Ticket / Bug ID *")
        with q2:
        ...
        with q4:
            priority = st.selectbox(
                "Priority *", ["Critical", "High", "Medium", "Low"], index=2
            )
        reported_date = st.date_input("Reported Date *", value=date.today())
        description = st.text_area(
            "Requirement Gap / Bug Description *", height=100
        )
        ...
        status = st.selectbox(
            "Status (Closed / Pending) *", ["Pending", "Closed"], index=0
        )
    ...
    if qa_submit:
        try:
            obj = Issue(
                ...
                reported_date=datetime.combine(
                    reported_date, datetime.min.time()
                ),
                reported_by=reported_by.strip(),
                priority=priority,
                description=description.strip(),
                steps_to_reproduce=(steps.strip() if steps else None),
                impacted_functionality=(
                    impacted.strip() if impacted else None
                ),
                root_cause_identified=(
                    rc_ident.strip() if rc_ident else None
                ),
                rca_notes=(rca_notes.strip() if rca_notes else None),
                short_term_fix=(stf.strip() if stf else None),
                long_term_preventive_action=(
                    lpa.strip() if lpa else None
                ),
                responsible_person=(resp.strip() if resp else None),
                target_completion_date=(
                    datetime.combine(tcd, datetime.min.time())
                    if tcd
                    else None
                ),
                verified_by=(ver_by.strip() if ver_by else None),
                verification_date=(
                    datetime.combine(ver_dt, datetime.min.time())
                    if ver_dt
                    else None
                ),
                status=status,
    ...
    cols = [
        ...
        "Reported By",
        "Priority",
        "Requirement Gap / Bug Description",
        "Steps to Reproduce",
    ...
    if "editor_df" not in st.session_state:
        st.session_state.editor_df = pd.DataFrame(
            [{c: "" for c in cols} for _ in range(5)]
        )
    upl = st.file_uploader(
        "Upload Excel/CSV into editor (not DB yet)",
        type=["xlsx", "xls", "csv"],
        key="upl_editor",
    )
    if upl is not None:
        try:
            ...
            if missing:
                ...
                st.session_state.editor_df = data.copy()
                st.success(f"Loaded {len(data)} rows into editor.")
        except Exception as e:
            st.error(f"Failed to load file: {e}")
    with st.form("editor_form", clear_on_submit=False):
        add_empty = st.form_submit_button("➕ Add Empty Row")
        if add_empty:
            st.session_state.editor_df.loc[
                len(st.session_state.editor_df)
            ] = {c: "" for c in cols}
        edited = st.data_editor(
            ...
            column_config={
                ...
                "Status (Closed / Pending)": st.column_config.SelectboxColumn(
                    options=["Pending", "Closed"], default="Pending"
                ),
            },
        )
        save_btn = st.form_submit_button(
            "💾 Save All Editor Rows to DB (Append)"
        )
    if save_btn:
        try:
            added, skipped = 0, 0
            for _, r in edited.iterrows():
                ticket = r.get("Ticket / Bug ID")
                module = r.get("Module Name")
                desc = r.get("Requirement Gap / Bug Description")
                rep_by = r.get("Reported By")
                rep_dt = parse_dt(r.get("Reported Date"), default_today=True)
                pri = r.get("Priority") or "Medium"
                status_val = r.get("Status (Closed / Pending)") or "Pending"
                if not ticket or not module or not desc or not rep_by:
                    skipped += 1
                    continue
                obj = Issue(
                    ticket_id=str(ticket).strip(),
                    module_name=str(module).strip(),
                    reported_date=rep_dt,
                    reported_by=str(rep_by).strip(),
                    priority=str(pri).strip(),
                    description=str(desc).strip(),
                    steps_to_reproduce=(
                        None
                        if pd.isna(r.get("Steps to Reproduce"))
                        else str(r.get("Steps to Reproduce"))
                    ),
                    impacted_functionality=(
                        None
                        if pd.isna(r.get("Impacted Functionality"))
                        else str(r.get("Impacted Functionality"))
                    ),
                    root_cause_identified=(
                        None
                        if pd.isna(r.get("Root Cause Identified"))
                        else str(r.get("Root Cause Identified"))
                    ),
                    rca_notes=(
                        None
                        if pd.isna(r.get("Root Cause Analysis Notes"))
                        else str(r.get("Root Cause Analysis Notes"))
                    ),
                    short_term_fix=(
                        None
                        if pd.isna(r.get("Short-term Fix"))
                        else str(r.get("Short-term Fix"))
                    ),
                    long_term_preventive_action=(
                        None
                        if pd.isna(r.get("Long-term Preventive Action"))
                        else str(r.get("Long-term Preventive Action"))
                    ),
                    responsible_person=(
                        None
                        if pd.isna(r.get("Responsible Person"))
                        else str(r.get("Responsible Person"))
                    ),
                    target_completion_date=parse_dt(
                        r.get("Target Completion Date")
                    ),
                    verified_by=(
                        None
                        if pd.isna(r.get("Verified By"))
                        else str(r.get("Verified By"))
                    ),
                    verification_date=parse_dt(
                        r.get("Verification Date")
                    ),
                ...
                added += 1
            session.commit()
            st.success(f"Saved {added} rows ✅ | Skipped: {skipped}")
        except Exception as e:
            session.rollback()
...
# ---------- Manage Issues (Dark) ----------
    ...
    df = get_live_df()
    all_count = len(
        to_df(session.execute(select(Issue)).scalars().all())
    )
    ...
    if df.empty:
        ...
        ids = df["ID"].tolist()
        sel = st.selectbox("Select ID to Edit", ids if ids else [None])
        if sel:
            item = session.get(Issue, int(sel))
            with st.form("edit_form"):
                e1, e2, e3, e4 = st.columns(4)
                with e1:
                    item.ticket_id = st.text_input(
                        "Ticket / Bug ID *", value=item.ticket_id or ""
                    )
                with e2:
                    item.module_name = st.text_input(
                        "Module Name *", value=item.module_name or ""
                    )
                with e3:
                    rd = (
                    ...
                    item.reported_date = datetime.combine(
                        rd_new, datetime.min.time()
                    )
                with e4:
                    item.reported_by = st.text_input(
                        "Reported By *", value=item.reported_by or ""
                    )
                ...
                with f1:
                    item.priority = st.selectbox(
                        ...
                        index=[
                            ...
                            "Medium",
                            "Low",
                        ].index(item.priority or "Medium"),
                    )
                with f2:
                    item.status = st.selectbox(
                        "Status (Closed / Pending) *",
                        ["Pending", "Closed"],
                        index=["Pending", "Closed"].index(
                            item.status or "Pending"
                        ),
                    )
                with f3:
                    item.responsible_person = st.text_input(
                        "Responsible Person",
                        value=item.responsible_person or "",
                    )
                with f4:
                    tcd = (
                        item.target_completion_date.date()
                        if item.target_completion_date
                        else None
                    )
                    tcd_new = st.date_input(
                        "Target Completion Date", value=tcd
                    )
                    item.target_completion_date = (
                        datetime.combine(tcd_new, datetime.min.time())
                        if tcd_new
                        else None
                    )
                ...
                item.steps_to_reproduce = st.text_area(
                    "Steps to Reproduce",
                    value=item.steps_to_reproduce or "",
                    height=110,
                )
                item.impacted_functionality = st.text_area(
                    "Impacted Functionality",
                    value=item.impacted_functionality or "",
                    height=100,
                )
                g1, g2 = st.columns(2)
                with g1:
                    item.root_cause_identified = st.text_input(
                        "Root Cause Identified",
                        value=item.root_cause_identified or "",
                    )
                    item.short_term_fix = st.text_area(
                        "Short-term Fix",
                        value=item.short_term_fix or "",
                    )
                with g2:
                    item.rca_notes = st.text_area(
                        "Root Cause Analysis Notes",
                        value=item.rca_notes or "",
                    )
                    item.long_term_preventive_action = st.text_area(
                        "Long-term Preventive Action",
                        value=item.long_term_preventive_action or "",
                    )
                h1, h2 = st.columns(2)
                with h1:
                    item.verified_by = st.text_input(
                        "Verified By", value=item.verified_by or ""
                    )
                with h2:
                    vd = (
                        item.verification_date.date()
                        if item.verification_date
                        else None
                    )
                    vd_new = st.date_input(
                        "Verification Date", value=vd
                    )
                    item.verification_date = (
                        datetime.combine(vd_new, datetime.min.time())
                        if vd_new
                        else None
                    )
                colA, colB = st.columns([1, 1])
                save = colA.form_submit_button("💾 Save")
                delete = colB.form_submit_button("🗑️ Delete")
            if save:
                session.commit()
                st.success("Saved ✅")
                st.rerun()
...
# ---------- Import / Export (Light) ----------
    ...
    with pd.ExcelWriter(out_tpl, engine="openpyxl") as writer:
        pd.DataFrame(columns=cols).to_excel(
            writer, index=False, sheet_name="Template"
        )
    st.download_button(
        "📄 Download Import Template",
        data=out_tpl.getvalue(),
        file_name="rca_import_template_v4_8.xlsx",
    )
    upl2 = st.file_uploader(
        "Upload Excel/CSV to import into DB (Replace or Append)",
        type=["xlsx", "xls", "csv"],
        key="upl_backup",
    )
    if upl2 is not None:
        data = (
            pd.read_csv(upl2)
            if upl2.name.lower().endswith(".csv")
            else pd.read_excel(upl2)
        )
        ...
        if col1.button("➕ Append to DB"):
            added = rows_to_objects(data)
            st.success(f"Appended {added} rows ✅")
            st.rerun()
        if col2.button("♻️ Replace DB (wipe & load)"):
            session.execute(text("DELETE FROM issues"))
            session.commit()
            added = rows_to_objects(data)
            st.success(f"Replaced dataset with {added} rows ✅")
            st.rerun()
